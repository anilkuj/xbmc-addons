...
def ListTitles(section, html):
        ...
        if 'movies' in section:
                ...
                for url, img, title, imdbUrl in match:
                        try:
                                addon.add_directory({'mode': 'GetLinks', 'section': section, 'url': url}, {'title':  title}, img= img)
                        except:
                                continue
        else:
                print 'in else'
...
def Categories(section):  #categories
        ...
        for cat, title in match:
                ...
                if 'Complete BluRay' in title or '1080p' in title or '720p' in title or 'BDSCR' in title:
                        continue
