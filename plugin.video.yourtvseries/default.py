...
BASE_URL = 'http://www.yourtvseries.eu'
...
def GetLinks(url): # Get TV Links
        ...
        match = re.search("str2=.+?php(.+?)'.+?str3='([\w=&]+)", orig_html)
        if not match:
                print 'could not obtain video'
                return
        url = 'http://vk.com/video_ext.php' + match.group(1) + match.group(2)
        print url
        html = net.http_GET(url).content
...
def BrowseAll(): 
	print 'Browse All screen'
        ...
        match = re.compile('cat-item.+?href="(.+?)".+?>(.+?)<').findall(html)
...
def BrowseLatest():
	print 'Browse Latest screen'
        ...
        episodes = re.compile('wrap" href="(.+?)".+?data-src="(.+?)".+?b>(.+?)<.+?strong>(.+?)<.+?translate">(.+?)<').findall(html)
...
def GetSeasons(url):
	...
	print 'Seasons for TV Show %s' % url
        ...
        if match:
                img = match.group(1)
	shows = re.compile('mts">(.+?)<(.+?)</div></div></div></div></div>', re.DOTALL).findall(html)   
	if shows:
...
def GetEpisodes(episodes): 
	...
	episodes = re.compile('wrap" href="(.+?)".+?data-src="(.+?)".+?strong>(.+?)<.+?stat">(.+?)<', re.DOTALL).findall(episodes) 
	for epurl, epimg, epnum, title in episodes:
                title = epnum.strip() + ' : ' + title.encode('utf8').strip()
                addon.add_directory({'mode': 'GetLinks', 'url': epurl}, {'title':  title}, img= epimg)
	xbmcplugin.endOfDirectory(int(sys.argv[1]))
